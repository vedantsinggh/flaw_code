...
class IntelligentRouter:
    ...
    def route_task(self, task_description: str, task_type: str = "implementation") -> Dict[str, Any]:
        """
        ...
        # ── Dynamic Cost and Time estimate ─────────────────────────────────────
        # Average 4 characters per token
        input_tokens = int((len(task_description) + 3000) / 4) # system prompt is ~3000 chars
        output_tokens = int(expected_loc * 12) if task_type != "planning" else 5000
        est_tokens = input_tokens + output_tokens

        # Speed estimates (tokens/sec) based on model/provider
